...
pub fn kmer_pol<'a>(
    ...
    child_fp: &std::collections::HashMap<std::string::String, f64>,
    no_hits_num: usize,
    fp_correct: f64,
) -> &'a str {
    ...
    let p_false = child_fp.get(&colors_accession[&top_hit]).unwrap();
    ...
    c = reads
        ...
        .map(|r| {
            ...
            if (r.len() == 1) && (r[0].len() < k) {
                ...
                let map = kmer::kmerize_vector(r.to_vec(), k, d);
    ...
    c = reads
        ...
        .map(|r| {
            ...
            if (r.len() == 1) && (r[0].len() < k) {
                ...
                let map = kmer::minimerize_vector(r.to_vec(), k, m, d);
                let report = search_index(&child_bigsi, &map, bloom_size, num_hash, no_hits_num);
                if report.is_empty() {
                    "no_hits"
                } else {
...
pub fn per_read_stream_pe(
    ...
    for line in iter1 {
        ...
        if line_count % 4 == 2 {
            match line2 {
                Some(l2) => {
                    vec.push(vec![l.to_owned(), l2.unwrap().to_owned()]);
                }
                None => break,
            };
        }
...
pub fn per_read_stream_mini_pe(
    filenames: Vec<&str>,
    bigsi_map: &std::collections::HashMap<usize, Vec<u8>>, //has to be an Arc ?
    colors_accession: &std::collections::HashMap<usize, String>,
    ref_kmers_in: &std::collections::HashMap<String, usize>,
            ...
            c = vec
                ...
                .map(|r| {
                    ...
                    if (r.len() == 1) && (r[0].len() < k) {
                        ...
                        let map = minimerize_vector(r.to_vec(), k, m, d);
                        let report =
                            search_index(&child_bigsi, &map, bloom_size, num_hash, no_hits_num);
                        if report.is_empty() {
                            "no_hits"
...
pub fn per_read_stream_mini_se(
    filenames: Vec<&str>,
    bigsi_map: &std::collections::HashMap<usize, Vec<u8>>, //has to be an Arc ?
    colors_accession: &std::collections::HashMap<usize, String>,
    ref_kmers_in: &std::collections::HashMap<String, usize>,
    ...
    d: usize, //downsample factor
    fp_correct: f64,
    b: usize, // batch size of number of reads to be analyzed in parallel
) -> std::collections::HashMap<std::string::String, usize> {
    let search_time = SystemTime::now();
    let mut tax_map = HashMap::new();
    let no_hits_num: usize = colors_accession.len();
